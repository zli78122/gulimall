...
public class RabbitMQReceiver {
     ...
     *
     * 2.消费端确认 - Ack机制 (保证每条消息都被正确消费)
     *     默认是 消费端自动确认，只要接收到消息，客户端就会自动确认，然后 Broker 会从服务端移除这条消息
     *     消费端自动确认 存在问题 : 当客户端收到多条消息，在处理这些消息时，客户端宕机，这样会造成消息丢失，因为服务器中不会保存那些剩余还未被处理的消息
     *     解决方式 : 消费端手动确认
     *         只要客户端没有明确告诉服务器收到消息，消息就不能从RabbitMQ服务器中删除
     *         Consumer宕机，未被确认的消息的状态会从 Unacked -> Ready
     *         Consumer恢复(可以继续从 Queue 中读取消息了)，未被确认的消息的状态会从 Ready -> Unacked
     *
